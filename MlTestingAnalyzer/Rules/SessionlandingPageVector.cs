...
{
    public class SessionLandingPageVector : IRule
    {

        private List<BlobDataContract> _list;
        public SessionLandingPageVector(List<BlobDataContract> list)
        {
            _list = list;
        }
        public List<BlobDataContract> RuleForList(string[] countryKey, bool stat)
        {
            ...
            {
                var enterVectors = blob.session_landing_page_vector.Split(',');
