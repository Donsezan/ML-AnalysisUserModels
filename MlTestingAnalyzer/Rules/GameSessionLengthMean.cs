...
{
    public class GameSessionLengthMean : IRule
    {
        private List<BlobDataContract> _list;
        public GameSessionLengthMean(List<BlobDataContract> list)
        {
            _list = list;
        }
        public List<BlobDataContract> RuleForList(string[] countryKey, bool stat)
        {
            ...
            {
                var value = Convert.ToDouble(blob.game_session_length_mean);
